...
# 判断获取到的图片数量和标签数量是否一致，如果不一致即报错
if len(image_list) != len(label_list):
    exit('Error: the number of labels and the number of images are not equal!')
total_samples = len(image_list)

for plane in ['X', 'Y', 'Z']:
    # list_training[plane]是一个文件存放了三个文件training_X.txt,training_Y.txt,training_Z.txt
...
for i in range(total_samples):
    ...
    for plane in ['X', 'Y', 'Z']:
        ...
        # 返回给定形状和类型的用0填充的数组
        # 返回slice_number行，organ_number+1列类型为int的用0填充的数组
        ...
        # 返回单行数组，元素个数为slice_number，类型为float
        ...
        for j in range(0, slice_number):
            image_filename_ = os.path.join(image_path_[plane], image_filename[i], '{:0>4}'.format(j) + '.npy')
            label_filename_ = os.path.join(label_path_[plane], label_filename[i], '{:0>4}'.format(j) + '.npy')
